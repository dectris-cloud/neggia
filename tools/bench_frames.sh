#!/usr/bin/env bash
# tools/bench_frames.sh — benchmark harness for the neggia plugin.
# Times plugin_open / plugin_get_header / a serial ascending frame loop
# (the plugin's caller reads frames serially in ascending order) via a
# dlopen runner sharing the pattern of tools/regress_bitexact.sh.
# CSV to stdout, diagnostics to stderr. CLOCK_MONOTONIC, ns internal, ms out.
#
# Usage:
